...
export function Step2Wedding({ formData, updateFormData }: StepProps) {
          ...
          {toggles.map((t) => (
            <button
              ...
              className={`flex flex-col items-center justify-center p-5 rounded-3xl border transition-all duration-500 group ${
                (formData as Record<string, boolean | undefined>)[t.key] 
            ...
            >
              <div className={`mb-3 transition-colors duration-500 ${
                (formData as Record<string, boolean | undefined>)[t.key] ? 'text-[#A27B5C]' : 'text-gray-300 group-hover:text-[#E7D2CC]'
              }`}>
                {t.icon}
              </div>
              <span className={`text-[8px] font-bold uppercase tracking-[0.2em] text-center leading-tight ${
                (formData as Record<string, boolean | undefined>)[t.key] ? 'text-white' : 'text-gray-400'
              }`}>
                {t.label}
              </span>
              <div className={`mt-3 w-1 h-1 rounded-full transition-all duration-500 ${
                (formData as Record<string, boolean | undefined>)[t.key] ? 'bg-[#A27B5C] scale-150' : 'bg-transparent'
              }`}></div>
            </button>
          ))}
        </div>
