...
import type { DashboardData, RSVPRecord, DietaryRestrictions } from '@/app/actions/dashboard';
...
export function ExportModal({ isOpen, onClose, data }: ExportModalProps) {
  ...
  const generatePDFContent = () => {
    ...
    // Count music suggestions
    const musicSuggestions = confirmedGuests.filter(g => g.music).length;
...
<body>
    <div class="header">
        ...
        <p class="subtitle">Reporte Completo de Invitados</p>
        <p class="subtitle">Fecha: ${timestamp}</p>
        <p class="subtitle">Evento: ${data.invitation.headline || 'Mi Evento'}</p>
    </div>
    ...
    <div class="summary">
        ...
        <div class="summary-grid">
            <div class="summary-item">
                <div class="summary-number">${data.rsvps.length}</div>
                <div class="summary-label">Familias</div>
            </div>
            <div class="summary-item">
            ...
            <div class="summary-item">
                <div class="summary-number">${totalChildren}</div>
                <div class="summary-label">Niños</div>
            </div>
            <div class="summary-item">
                <div class="summary-number">${musicSuggestions}</div>
                <div class="summary-label">Sugerencias</div>
            </div>
            <div class="summary-item">
  ...
  const guestData = processGuestData();
  const confirmedGuests = guestData.filter(g => g.status === 'Confirmado');
  const totalGuests = confirmedGuests.reduce((sum, g) => sum + g.guests, 0);
  const guestsWithRestrictions = confirmedGuests.filter(g => g.restrictions !== 'Ninguna').length;
  const allergicGuests = confirmedGuests.filter(g => g.hasAllergies).length;
      ...
      <div className="relative bg-white rounded-3xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-stone-100">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-serif text-stone-800">Exportar Datos Completos</h2>
            <button
              onClick={onClose}
        ...
        <div className="p-6">
          ...
          <div className="bg-stone-50 rounded-xl p-4 mb-6">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <span className="text-stone-500">Total familias:</span>
                <span className="ml-2 font-medium text-stone-800">{data.rsvps.length}</span>
              </div>
              <div>
              ...
              <div>
                <span className="text-stone-500">Con música:</span>
                <span className="ml-2 font-medium text-blue-600">{confirmedGuests.filter(g => g.music).length}</span>
              </div>
            </div>
          </div>
          ...
          <div className="mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              >
                ...
                <span className="text-xs text-stone-400 block">Reporte completo</span>
              </button>
          ...
          <button
            ...
            disabled={isExporting || data.rsvps.length === 0}
